...
\lhead{LC\&B - 02.1 Computing - Conor}
\rhead{\thepage}
\cfoot{}
...
font (\texttt{fixed width font}) just before the code listing. It also
gives an introduction to Julia, you can use either, both or neither
language, so use these notes appropriately.
...
\item \textsl{torch} also called \textsl{pytorch}; a popular package
  for doing deep learning, another is \textsl{tensorflow}.
\end{itemize}
Depending on how you installed Python, many of these packages will
already be available or you may be able to install them using the tool
\texttt{pip3}.
...
This allows a function to act on a vector, so
\begin{lstlisting}[numbers=right]
  x=pi/2
  sin(x)
\end{lstlisting}
gives \texttt{1.0} but the broadcast form, which has an extra dot
\begin{lstlisting}[numbers=right]
  x=[pi,pi/2,pi/3]
  sin.(x)
\end{lstlisting}
gives \texttt{[0.0,1.0,0.8661]}. The dot `broadcasts' the function over the vector, promoting a function that is defined for ordinary numbers to one for vectors. This works for user-defined functions
\begin{lstlisting}[numbers=right]
  x=[-1,0,1]
  f(x)=x^2
  f.(x)
\end{lstlisting}
gives \texttt{[1,0,1]} when $f(x)$ would give an error.
...
Like Python, Julia has many libraries, called packages; in fact, Julia packages tend to
be smaller and so you end up using more of them in different
combinations. One difference, though, is that matrices are found in
the main part of Julia and so some of the things you use
\texttt{numpy} for in Python don't need a package in Julia and Julia
vectors and arrays can be treated much like \texttt{numpy} vectors and arrays.

To add packages in Julia you first enter the REPL, you type \texttt{]}
  to enter the mode which looks after packages; then \texttt{add
    Package} where \texttt{Package} is the name of the package you
  want to add. You leave the package manager mode by pressing
  backspace. To use a package you use \texttt{import}.

  Some popular and important libraries include
  \begin{itemize}
  \item \textsl{DifferentialEquations} provides numerical integration of differential equations; support for solving differential equations in Julia is superb.
    \item \textsl{turing.jl} does probabilistic programming which is
      used for modelling probabalistic processes.
    \item \textsl{Gadfly} does plotting and is particularly good for presenting statistical data.
    \item \textsl{PyPlot} allow you to use \texttt{matplotlib} through Julia.
    \item \textsl{Dataframes} helps with handling large or complicated datasets.
      \end{itemize}
